...
export function getAptaxServerEnv() {
  const result = envSchema.safeParse({
    ...
    NEXT_PUBLIC_APTAX_REGISTRY_ADDRESS: process.env.NEXT_PUBLIC_APTAX_REGISTRY_ADDRESS,
    NEXT_PUBLIC_APTAX_METRIC_STORE_ADDRESS:
      process.env.NEXT_PUBLIC_APTAX_METRIC_STORE_ADDRESS,
    NEXT_PUBLIC_APTAX_VERIFIER_ADDRESS: process.env.NEXT_PUBLIC_APTAX_VERIFIER_ADDRESS,
  });
  if (!result.success) {
    const message = result.error.issues.map((issue) => issue.message).join(" ");
    throw new Error(
      `Aptax env configuration is incomplete. Set NEXT_PUBLIC_NETWORK_CHAIN_ID, NEXT_PUBLIC_NETWORK_RPC_URL, NEXT_PUBLIC_APTAX_REGISTRY_ADDRESS, NEXT_PUBLIC_APTAX_METRIC_STORE_ADDRESS, and NEXT_PUBLIC_APTAX_VERIFIER_ADDRESS. Legacy NEXT_PUBLIC_APTAX_CHAIN_ID and NEXT_PUBLIC_APTAX_RPC_URL are still accepted for compatibility. ${message}`
    );
  }
